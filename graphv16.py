...
class Circiut(object):
    ...
    def gather_abc(self, flows, unknown): # WILL ONLY WORK WITH ONE UNKNOWN
        a, b, c = 1
        for flow in flows:
            c *= flow[2]
            a_temp, b_temp = 1
            for f in flows:
                if f is not flow:
                    a_temp *= f[2]
                    b_temp *= f[2]
            # don't want to check this every iteration!
            if unknown == 'a':
                a += a_temp
                b += flow[1]*b_temp
            elif unknown == 'b':
    ...
    def get_flows(self, type, node, time, special_nodes, solution_vars):
        ...
        for edge in self.circ.node[node][1][type]:
            if edge[3]['type'] == 'i':
                if bool(edge[3]['flow']):
                    floats += edge[3]['flow']
                else: # this should never be called right? because the previous rounds will all be calculated
                    self.calc_i(edge, time, special_nodes, solution_vars)
            elif edge[3]['type'] == 'r':
    ...
    def calc_pressure(self, node, time, special_nodes, solution_vars):
        ...
        self.circ.node[node]['calculated_p'] = pressure
        return pressure
    ...
    # make sure to extract the full edges in the functions from the into and out lists like in calc_c_flow
        ...
        # flow doesn't need to be coupled to time because you want to calculate with last cycle's value & it will always get updated every single time
        self.circ.edge[edge[0]][edge[1]][edge[2]]['flow'] = (p_in-p_out)/float(edge[3]['value'])
