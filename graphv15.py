...
class Circiut(object):
    ...
    def __init__(self):
        # consider also making special_nodes and solution_vars global
    ...
    def get_flows(self, type, node):
        for edge in self.circ.node[node][1][type]:
            if edge[3]['type'] == 'i':
            elif edge[3]['type'] == 'r':
    ...
    def calc_pressure(self, node, time, special_nodes, solution_vars):
        ...
        flows_out = []
        for x in ['into', 'out']:
            for edge in self.circ.node[node][x]:
                if not:
                    if i:
                        get ['flow']
                else:
    ...
    def is_unknown(self, node, time, solution_vars, special_nodes): # this is really similar to code in boundary_conditions, can you consolidate?
        # somehow check if value is saved from previously in the round <- will it only be used by capacitors here or will resistors need to access it as well?????
        if self.circ.node[node]['p'] != None:
            return self.boundary_conditions(time, node, solution_vars, special_nodes)
    ...
    # make sure to extract the full edges in the functions from the into and out lists like in calc_c_flow
        p_in = self.is_unknown(edge[0], time, solution_vars, special_nodes)
        p_out = self.is_unknown(edge[1], time, solution_vars, special_nodes)
    ...
    def boundary_conditions(self, time, node, special_nodes, solution_vars):
        if node in special_nodes:
            ...
            if time in self.circ.node[node]['p']:
                ...
                return np.interp(float(time), self.circ.node[node]['x'], self.circ.node[node]['y'], period=1)
        elif isinstance(self.circ.node[node]['p'], float): # this doesn't need to be altered, right?
            return self.circ.node[node]['p']
        # elif pressure has been saved and calculated before for this time cycle!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        else:
